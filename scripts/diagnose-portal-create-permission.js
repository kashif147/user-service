...
async function diagnosePortalCreatePermission(userEmail = null) {
  try {
    ...
    if (portalCreatePermission.resource.toLowerCase() !== "portal") {
      console.log(`⚠️  WARNING: Permission resource is "${portalCreatePermission.resource}" but should be "portal"`);
    }
    if (portalCreatePermission.action.toLowerCase() !== "create") {
      console.log(`⚠️  WARNING: Permission action is "${portalCreatePermission.action}" but should be "create"`);
    }
    ...
    portalPermissions.forEach((perm) => {
      ...
      console.log(`  • ${perm.code} - ${perm.name} (${perm.resource}:${perm.action})${marker}`);
    });
    console.log();
    ...
    for (const tenant of tenants) {
      ...
      if (!nonMemberRole) {
        ...
        console.log(`   Total Permissions: ${nonMemberRole.permissions.length}`);
        ...
        const portalPerms = nonMemberRole.permissions.filter(p => 
          typeof p === "string" && (p.includes("PORTAL") || p.includes("portal"))
        );
    ...
    if (userEmail) {
      ...
      if (!user) {
        ...
        const portalUserPerms = userPermissions.filter(p => 
          typeof p === "string" && (p.includes("PORTAL") || p.includes("portal"))
        );
        ...
        user.roles.forEach((role) => {
          ...
          console.log(`     • ${role.code} (${role.name}) - PORTAL_CREATE: ${hasPortalCreate ? "✅" : "❌"}`);
        });
      }
    }
    ...
    console.log("📊 SUMMARY");
    console.log("=".repeat(60));
    console.log(`✅ PORTAL_CREATE permission exists: ${portalCreatePermission ? "YES" : "NO"}`);
    console.log(`   Resource: ${portalCreatePermission.resource}`);
    console.log(`   Action: ${portalCreatePermission.action}`);
    console.log(`   Code: ${portalCreatePermission.code}`);
    console.log("\n💡 To fix missing PORTAL_CREATE in NON-MEMBER role:");
    console.log("   Run: NODE_ENV=staging node scripts/add-portal-create-to-non-member.js");
    console.log("=".repeat(60));
